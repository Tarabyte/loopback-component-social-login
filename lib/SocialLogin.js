...
class SocialLogin {
  ...
  static guessMode(name) {
    ...
    if (name.includes('login')) {
      debug(`Since ${name} includes "login" using authenticate mode.`);
      return AUTHENTICATE;
    }
    if (name.includes('signup')) {
      debug(`Since ${name} includes "signup" using authenticate mode.`);
      return AUTHENTICATE;
    }

    if (name.includes('link')) {
      debug(`Since ${name} includes "link" using authorize mode.`);
      return AUTHORIZE;
    }

    if (name.includes('connect')) {
      debug(`Since ${name} includes "connect" using authorize mode.`);
      return AUTHORIZE;
    }
